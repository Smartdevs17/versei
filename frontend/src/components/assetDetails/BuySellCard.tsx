...
export interface BuySellCardProps {
  ...
  onClose?: () => void;
  onBuyAssetClick?: () => void; // Added prop
}

const BuySellCard: React.FC<BuySellCardProps> = ({
  initialTab = "Buy",
  asset,
  onBuyAssetClick, // Destructure the prop
}) => {
  ...
  const pricePerUnit = asset
    ? parseFloat(asset.price.replace(" USDT", ""))
    : 200;
      ...
      {activeTab === "Buy" && (
        <div className="space-y-4">
          ...
          <div>
            ...
            <p className="w-full p-2 rounded bg-gray-100">
              {asset ? `${pricePerUnit} USDT` : "$200"}
            </p>
          </div>
          <div>
          ...
          <button
            ...
            onClick={onBuyAssetClick} // Use the prop
          >
            Buy Asset
          </button>
        </div>
